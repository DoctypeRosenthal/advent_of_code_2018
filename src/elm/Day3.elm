module Day3 exposing (claimId, claimToPoints, claimsIntersect, getSeparateClaim, overlapsBetweenClaims, parseClaim, part1, part2)

import Array exposing (Array)
import Set exposing (Set)
import Untils exposing (cycleLeft)
...
claimsIntersect : Claim -> Claim -> Set Point2D
claimsIntersect a b =
    Set.intersect (claimToPoints a) (claimToPoints b)


allIntersections : List Claim -> List (Set Point2D)
allIntersections claims =
    case claims of
        first :: rest ->
            rest
                |> List.map (\x -> claimsIntersect first x)
                |> List.append (allIntersections rest)
...
overlapsBetweenClaims : String -> Set Point2D
overlapsBetweenClaims str =
    str
        |> toClaims
        -- first calc intersections between all claims
        |> allIntersections
        -- then make union of the intersections
        |> List.foldl Set.union Set.empty


toClaims : String -> List Claim
toClaims str =
    str
        |> String.lines
        |> List.map (String.trim >> parseClaim)
...
part1 =
    Set.size <| overlapsBetweenClaims question


getSeparateClaim : String -> Maybe Claim
getSeparateClaim str =
    str
        |> toClaims
        |> findSeparate


claimId : Maybe Claim -> Maybe Int
claimId claim =
    case claim of
        Just { id } ->
            Just id
...
part2 =
    getSeparateClaim question |> claimId
